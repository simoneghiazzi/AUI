...
{
    public event Action<Dictionary<PartToTrack, bool>> HandState;
    public event Action<string[]> GestureState;
    ...
    void Update()
    {
        if (skelPosition != null)
        {
            foreach (var p in trackingSchema)
            {
                ...
                if (activateHandCloseEvents)
                {
                   ...
                   bool haschangehappened = false;
                    if ((skelPosition.IsLeftHandClosed() || Input.GetMouseButtonDown(0)) && !leftHandState)
                    {
                        hand.Add(PartToTrack.LeftHand, true);
                        leftHandState = true;
                        haschangehappened = true;
                    }
                    if ((!skelPosition.IsLeftHandClosed() || Input.GetMouseButtonUp(0)) && leftHandState)
                    {
                        hand.Add(PartToTrack.LeftHand, false);
                        leftHandState = true;
                        haschangehappened = true;
                    }
                    if ((skelPosition.IsRightHandClosed() || Input.GetMouseButtonDown(1)) && !rightHandState)
                    {
                        hand.Add(PartToTrack.RightHand, true);
                        rightHandState = true;
                        haschangehappened = true;
                    }
                    if ((!skelPosition.IsRightHandClosed() || Input.GetMouseButtonUp(1)) && rightHandState)
                    {
            ...
            if (activateGestureEvents)
            {
                ...
                {
                    GestureState?.Invoke(skelPosition.Gestures);
                }
            }
        }
    }
